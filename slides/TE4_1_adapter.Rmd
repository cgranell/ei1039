...
## objetos con interfaces .heatinline[incompatibles] pasen a ser .coldinline[compatibles]
...
### `Client` y `Service` están completamente _desacoplados_
...
???
The adapter essentially encapsulates the adaptee and presents a new interface to the client class. It does this by wrapping the adaptee’s interface and exposing a new target interface that makes sense to the client

---
# Adapter: Aplicabilidad

.large[`r fontawesome::fa(name = "arrow-circle-right")` Cuando incorporas clases existente de terceros (libreria, servicios, etc.), pero cuya interfaz no es compatible con tu código] 
