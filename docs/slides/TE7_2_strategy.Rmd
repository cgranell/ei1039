---
title: "[TE7_2] Patrones de Comportamiento"
...
> .large[Existe acoplamiento (_coupling_) si al cambiar un módulo en un programa requiere cambiar otro módulo. Acoplamiento impacta negativamente en la mantenibilidad del sistema]
...
[Programar a una interfaz, no a una implementación](https://cgranell.github.io/ei1039/slides/TE3_1_patrones_diseno.html#29) reduce el acoplamiento porque definir una interfaz que sirve como hub de comunicación es una buena estrategia para romper dependencias entre clases y módulos.
...
-   .large[Comprimir archivos usando diferentes algoritmos de compresión]
-   .large[Elegir entre diferentes algoritmos de panificación de rutas]
...
.large[`r fontawesome::fa(name = "arrow-circle-right")` cuando se utilizan variantes de un algoritmo y queremos cambiar de un algoritmo a otro dinámicamente]

--

.large[`r fontawesome::fa(name = "arrow-circle-right")` cuando tenemos numerosos `if-else` para seleccionar variantes de un mismo algoritmo (¡reemplazandola por una única llamada a un método!)]

--

.large[`r fontawesome::fa(name = "arrow-circle-right")` cuando queremos esconder detalles irrelevantes de la implementación de los algoritmos a los clientes]
...
.large[`r fontawesome::fa(name = "arrow-circle-right")` encapsula lo que varía (algoritmos) en clases separadas]

.large[`r fontawesome::fa(name = "arrow-circle-right")` reduce acoplamiento, sustituyendo herencia por composición]

.large[`r fontawesome::fa(name = "arrow-circle-right")` proporciona un mecanismo dinámico para cambiar algoritmos enteros mediante composición y delegación]
